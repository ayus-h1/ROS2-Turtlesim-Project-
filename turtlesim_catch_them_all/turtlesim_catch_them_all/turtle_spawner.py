...
class TurtleSpawnerNode(Node): 
    def __init__(self):
        ...
        self.alive_turtles_ = []
        self.spawn_new_turtle_timer_ = self.create_timer(1.0/self.spawn_frequency,self.spawn_new_turtle)
        self.alive_turtles_publisher_ = self.create_publisher(TurtleArray,"alive_turtles",10)
        self.turtle_catch_server_ = self.create_service(CatchTurtle,"catch_turtle", self.callback_catch_turtle)
    ...
    def callback_call_spawn(self,future,turtle_name,x,y,theta):
        try:
            ...
            if response.name != "":
                self.get_logger().info("turtle name: " + response.name + " is alive.")
                new_turtle = Turtle()
